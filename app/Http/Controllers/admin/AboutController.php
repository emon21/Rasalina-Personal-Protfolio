...
class AboutController extends Controller
{
    ...
    public function StoreMultiImage(Request $request,About $about){
        ...
        if ($request->hasFile('multiImage')) {
            ...
            foreach ($images as $image) {
                ...
                $image->move(public_path('uploads/about/multiImg/'), $filename);
                // only file upload no foldername in database
                $path = 'uploads/about/multiImg/' . $filename;
                // $uploadedFiles[] = $path;
        ...
        return redirect()->route('admin.all.multi.image');
    ...
    public function UpdateMultiImage(Request $request,$update){
        ...
        foreach ($image as $multiImage) {
            ...
            # upload Image
            $filename = time().'_'.rand(100000, 999999) . '.' . $multiImage->getClientOriginalExtension();
            $multiImage->move(public_path('uploads/about/multiImg/'), $filename);
        ...
        // ];
        // return redirect()->route('about.multi.image');
        return redirect()->route('admin.about.multi.image')->with('files', $uploadedFiles);
